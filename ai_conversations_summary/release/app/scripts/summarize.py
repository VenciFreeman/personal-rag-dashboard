...
def request_chat_completion(
	*,
	...
	# Match DeepSeek usage: OpenAI SDK + custom base_url.
	...
	while attempt < max_attempts:
		...
		try:
			...
			print(
				f"Transient API error (attempt {attempt}/{max_attempts}): {exc}. "
			...
			if status_code is not None and status_code >= 500 and attempt < max_attempts:
				wait_seconds = retry_delay * (2 ** (attempt - 1))
				print(
					f"Server status {status_code} (attempt {attempt}/{max_attempts}). "
					f"Retrying in {wait_seconds:.1f}s..."
				)
				time.sleep(wait_seconds)
				continue
...
def extract_date_prefix_from_source(source_text: str) -> str:
	# Read date from the beginning `updated_at` field, fallback to today's date.
...
def main() -> None:
	...
	for file_path in files:
		...
		if not source_text:
			...
			continue
		date_prefix = extract_date_prefix_from_source(source_text)
