...
def _find_local_snapshot_path(model_name: str) -> Path | None:
    ...
    cache_root = Path(__file__).resolve().parent.parent / "data" / "local_models"
    if not cache_root.exists():
        return None

    pattern = f"**/models--{safe_id}/snapshots/*"
    candidates = [p for p in cache_root.glob(pattern) if p.is_dir()]
    if not candidates:
        return None
...
def _find_local_model_dir_candidates(model_name: str) -> list[Path]:
    ...
    local_models_root = workspace_root / "data" / "local_models"
...
def build_topic_records(documents_dir: Path) -> list[TopicRecord]:
    ...
    for file_path in sorted(documents_dir.rglob("*.md")):
        ...
        emb_text = f"keywords: {keyword_text}\nsummary_topics: {topic_summary}".strip()
        records.append(
            TopicRecord(
                id=rec_id,
                file_path=rel,
...
def _embed_texts_local(
    ...
    *,
    ...
    if encoder is None:
        ...
        attempts.append((model_name, str(Path(__file__).resolve().parent.parent / "data" / "local_models")))
...
def search_vector_index(
    *,
    ...
    if selected_backend == "faiss":
        return _search_faiss(index_dir, query_vector, top_k)
    if selected_backend == "chroma":
...
def search_vector_index_with_diagnostics(
    *,
    ...
    if selected_backend == "faiss":
        ...
        results = _search_faiss(index_dir, query_vector, top_k)
