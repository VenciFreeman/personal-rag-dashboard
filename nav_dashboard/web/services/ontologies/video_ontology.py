"""Video domain ontology wrapper — parallel to music_ontology.py.

Exposes helpers for genre/creator/entity hint extraction from query text,
backed by ``ontologies/approved/video.json`` via the shared ``ontology_loader``.

Functions consumed by the agent pipeline
-----------------------------------------
* ``load_video_ontology()``          — raw dict (mtime-reloaded)
* ``collect_video_ontology_hints()`` — structured signal dict for query enrichment
* ``infer_video_filters_from_text()``— filter dict compatible with _infer_media_filters
...
def load_video_ontology() -> dict[str, Any]:
    """Return the approved video ontology, reloading automatically if the file changed."""
    try:
        return _load_ontology("video")
    except Exception as exc:
        _logger.warning(
            "ontology/video: failed to load (%s) — hint/filter functions will return empty results",
            exc,
        )
        return {
            "genres": {},
...
def collect_video_ontology_hints(text: str) -> dict[str, Any]:
    """Scan *text* for video-domain ontology matches and return structured hints.

    Returns a dict with keys:
        genre_keys, creator_keys, entity_keys         — matched canonical keys
        genre_hints, creator_hints, entity_hints      — flat alias lists
    """
    ontology = load_video_ontology()
...
def infer_video_filters_from_text(text: str) -> dict[str, list[str]]:
    """Return a filter dict (media_type, category) inferred from video ontology matches.

    The returned dict is compatible with the ``_merge_filter_values`` / ``_infer_media_filters``
    pipeline in ``agent_service.py``.
    """
    hints = collect_video_ontology_hints(text)
...
def is_genre_alias(token: str) -> bool:
    """Return True if *token* is a recognised genre alias in the video ontology."""
    ontology = load_video_ontology()
...
def is_entity_alias(token: str) -> bool:
    """Return True if *token* is a recognised entity/franchise alias in the video ontology."""
    ontology = load_video_ontology()
    normalized = _normalize_token(token)
    if not normalized:
        return False
