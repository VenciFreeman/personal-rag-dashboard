...
def _debug_log_path(hook_input: dict[str, Any]) -> Path | None:
    ...
    if not raw_path:
        return _default_ticket_dir(base) / "bug_ticket_hook_debug.jsonl"
    return _canonical_ticket_sidecar_path(_resolve_path(raw_path, base))
...
def _path_endswith(path: Path, parts: tuple[str, ...]) -> bool:
    lowered = tuple(item.lower() for item in path.parts)
    return len(lowered) >= len(parts) and lowered[-len(parts):] == parts


def _canonical_ticket_dir(path: Path) -> Path:
    resolved = path.resolve()
    if _path_endswith(resolved, ("data", "nav_dashboard", "tickets")):
        return resolved
    if _path_endswith(resolved, ("data", "nav_dashboard")):
        return (resolved / "tickets").resolve()
    if _path_endswith(resolved, ("nav_dashboard", "data", "tickets")):
        return resolved
    if _path_endswith(resolved, ("nav_dashboard", "data")):
        return (resolved / "tickets").resolve()
    if _path_endswith(resolved, ("records", "nav_dashboard")):
        return (WORKSPACE_ROOT / "data" / "nav_dashboard" / "tickets").resolve()
    return resolved


def _canonical_ticket_file(path: Path) -> Path:
    resolved = path.resolve()
    if resolved.name.lower() != "tickets.jsonl":
        return resolved
    return (_canonical_ticket_dir(resolved.parent) / "tickets.jsonl").resolve()


def _canonical_ticket_sidecar_path(path: Path) -> Path:
    resolved = path.resolve()
    if _path_endswith(resolved.parent, ("data", "nav_dashboard")):
        return (_canonical_ticket_dir(resolved.parent) / resolved.name).resolve()
    if _path_endswith(resolved.parent, ("nav_dashboard", "data")):
        return (_canonical_ticket_dir(resolved.parent) / resolved.name).resolve()
    if _path_endswith(resolved.parent, ("records", "nav_dashboard")):
        return (_canonical_ticket_dir(resolved.parent) / resolved.name).resolve()
    return resolved


def _default_ticket_dir(base: Path) -> Path:
    return (base / "data" / "nav_dashboard" / "tickets").resolve()
...
def _configure_ticket_file(hook_input: dict[str, Any]) -> Path:
    ...
    if explicit_file:
        ticket_file = _canonical_ticket_file(_resolve_path(explicit_file, base))
    elif explicit_dir:
        ticket_file = (_canonical_ticket_dir(_resolve_path(explicit_dir, base)) / "tickets.jsonl").resolve()
    else:
        ticket_file = _default_ticket_dir(base) / "tickets.jsonl"
...
def _outbox_file(ticket_file: Path) -> Path:
    ...
    if explicit_path:
        return _canonical_ticket_sidecar_path(_resolve_path(explicit_path, WORKSPACE_ROOT))
