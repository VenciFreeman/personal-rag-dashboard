...
Usage:
    python scripts/ontology_propose.py --domain music
    python scripts/ontology_propose.py --domain video --max-items 80 --max-traces 30
    python scripts/ontology_propose.py --domain book --dry-run
    python scripts/ontology_propose.py --domain music --source-type title_cluster
"""
...
TRACE_DATA_DIR = WORKSPACE_ROOT / "nav_dashboard" / "data"
...
def _load_recent_traces(max_traces: int, skip_ids: set[str] | None = None) -> list[dict[str, Any]]:
    ...
    legacy = TRACE_DATA_DIR / "trace_records.jsonl"
    if legacy.exists():
        files.append(legacy)
...
def main() -> None:
    ...
    print()
    print("Next step: run  python scripts/ontology_review.py --domain", domain)
