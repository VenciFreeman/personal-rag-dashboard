...
echo [Library Tracker] Preparing LAN deployment on %LIBRARY_WEB_HOST%:%LIBRARY_WEB_PORT% ...
...
echo [Library Tracker] Skipping firewall rule setup (non-admin mode).
goto FIREWALL_DONE
...
if defined PY_FOR_FIREWALL (
    netsh advfirewall firewall delete rule name="Library Tracker Python Inbound" >nul 2>nul
    netsh advfirewall firewall add rule name="Library Tracker Python Inbound" dir=in action=allow program="%PY_FOR_FIREWALL%" profile=any >nul 2>nul
)
netsh advfirewall firewall show rule name="Library Tracker Web %LIBRARY_WEB_PORT%" >nul 2>nul
if not %errorlevel%==0 (
    echo [WARN] Firewall rule was not created successfully.
    echo [WARN] Other computers may not be able to connect.
) else (
    echo [Library Tracker] Firewall rule ready: Library Tracker Web %LIBRARY_WEB_PORT%
    netsh advfirewall firewall show rule name="Library Tracker Web %LIBRARY_WEB_PORT%"
)
if defined PY_FOR_FIREWALL (
    netsh advfirewall firewall show rule name="Library Tracker Python Inbound" >nul 2>nul
    if %errorlevel%==0 (
        echo [Library Tracker] Firewall rule ready: Library Tracker Python Inbound
        netsh advfirewall firewall show rule name="Library Tracker Python Inbound"
    )
)

:FIREWALL_DONE

echo [Library Tracker] LAN access URLs (same subnet):
...
for /f "tokens=2 delims=:" %%a in ('ipconfig ^| findstr /R /C:"IPv4 Address" /C:"IPv4.*:"') do (
    set "ip=%%a"
    set "ip=!ip: =!"
    if not "!ip!"=="" echo   http://!ip!:%LIBRARY_WEB_PORT%/
)
endlocal
echo.
echo [Library Tracker] Keep this window open while serving. Press Ctrl+C to stop.
echo [Library Tracker] If another computer still cannot access:
...
echo   4) On another PC run: Test-NetConnection 192.168.0.102 -Port %LIBRARY_WEB_PORT%
